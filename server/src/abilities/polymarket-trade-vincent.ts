 ...
 *
 * Proper Vincent Ability implementation for Polymarket trading
 * This should eventually be published as @whispers/vincent-ability-polymarket-trade
 */
 ...
 * Defines the shape of successful precheck responses
 */
 ...
 * Defines the shape of failed precheck responses
 */
 ...
 * Defines the shape of successful execution responses
 */
...
/**
 * Execute Fail Schema
 * Defines the shape of failed execution responses
 */
...
export const vincentAbility = createVincentAbility({
  packageName: "@whispers/vincent-ability-polymarket-trade",
  abilityDescription:
    "Place buy and sell orders on Polymarket prediction markets",

  abilityParamsSchema,
  supportedPolicies: supportedPoliciesForAbility([]), // No policies for now
  ...
  precheck: async ({ abilityParams }, { succeed, fail }) => {
    try {
      ...
      // Get the delegator's PKP address from context
      // Note: In actual Vincent execution, this comes from the context
      ...
      // Calculate required USDC (for BUY orders)
   ...
   */
  execute: async ({ abilityParams }, { succeed, fail, pkpSign }) => {
    try {
      const { tokenId, side, price, amount } = abilityParams;

      // Get PKP signer from context
      // In actual Vincent execution, pkpSign is provided by the runtime
      ...
      // Submit order using CLOB client
...
/**
 * Export the ability
 * This is what gets imported by your backend
 */
